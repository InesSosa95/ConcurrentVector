public class SequentialVector {
     ...
     * @precondition size > 0.
     */
    public SequentialVector(int size) {
     ...
     */
    public void assign(SequentialVector v) {
        for (int i = 0; i < dimension(); ++i)
            set(i, v.get(i));
    }
     ...
     */
    public void assign(SequentialVector mask, SequentialVector v) {
        for (int i = 0; i < dimension(); ++i)
            if (mask.get(i) >= 0)
                set(i, v.get(i));
    }
     ...
     * @precondition dimension() == v.dimension().
     */
    public void add(SequentialVector v) {
        for (int i = 0; i < dimension(); ++i)
            set(i, get(i) + v.get(i));
    }
     ...
     * @precondition dimension() == v.dimension().
     */
    public void mul(SequentialVector v) {
        for (int i = 0; i < dimension(); ++i)
            set(i, get(i) * v.get(i));
    }
    ...
    public double prod(SequentialVector v) {
        SequentialVector aux = new SequentialVector(dimension());
        aux.assign(this);
        aux.mul(v);
        return aux.sum();
    }
